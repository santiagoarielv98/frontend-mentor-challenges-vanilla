...
import { getData } from "./services/rest-countries";
  ...
  <div class="flex-1 max-w-[37.5rem] mx-auto w-full">
    <div class="flex flex-col justify-center gap-8 h-full w-full">
      ...
      <div class="flex items-center flex-wrap gap-[9px] mt-9">
        ...
        ${country.borders
          ?.map((b) => `<span class="w-24 px-2 py-1 text-center font-semibold text-sm border rounded-md">${b}</span>`)
          .join("")}
      </div>
    </div>
  </div>
...
const renderHome = () => {
  ...
  <label class="inline-flex w-full max-w-[12.5rem] relative h-max shadow-md bg-white rounded-md">
    <input id="filter" hidden class="peer" type="checkbox" name="gender" />
    ...
    >
    <ul class="invisible absolute inset-x-0 top-full mt-2 py-3 peer-checked:visible shadow-md bg-white cursor-pointer">
...
const renderCountries = (countries: Country[]) => {
  ...
  countries.forEach((country) => {
    ...
    card.innerHTML = renderCard(country);
    card.classList.add("mx-auto");
    card.addEventListener("click", () => {
      app.innerHTML = renderDetailPage(country);
...
const initialRender = async () => {
  ...
  ul.addEventListener("click", onEvent);
  searchInput.addEventListener("input", onEvent);
};
